...
interface QuizModeProps {
  ...
  onUpdateSRS: (storyId: string, assocIdx: number, srs: SRSMetadata) => void;
...
const QuizMode: React.FC<QuizModeProps> = ({ quizData, mnemonicData, isInterleaved, reviewQueue, onExit, setHighlightIndex, onUpdateSRS, onCurrentItemChange, t }) => {
  ...
  const handleRating = (quality: number) => {
    ...
    // SM-2 logic
    const newSRS = calculateNextSRS(quality, currentItem.association.srs);
    onUpdateSRS(currentItem.storyId, currentQuestion!.associationIndex, newSRS);
